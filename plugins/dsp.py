...
def _op_uart_expect(session, h, args):
    ...
    while time.monotonic() < deadline:
        ...
        if stream.contains_bytes(sentinel, cursor):
            ...
            session.record_check(
                "uart_expect", "dsp.uart", claim, "hit",
    ...
    session.record_check(
        "uart_expect", "dsp.uart", claim, "timeout",
        {"sentinel": sentinel.decode("utf-8", "replace")})
...
class DspPlugin(DevicePlugin):
    ...
    def close(self, handle):
        ...
        # so the chip is already back at idle. Flush the UART
        # buffers before closing in case uart_write was interrupted
        # mid-byte.
