    ...
    def _run_inventory(self, op, plugins):
        ...
        ops_map["_jobs"] = {
            "doc": (
                ...
                "                         done), timestamps, and the\n"
                "                         per-submission meta dict.\n"
                "  DELETE /jobs/<digest>  cancel. If still queued, the\n"
                "                         server unlinks it before the\n"
                "                         poller can pick it up\n"
                "                         (returns canceled_queued). If\n"
                "                         in-flight, the server drops a\n"
                "                         marker; the poller signals\n"
                "                         the running session to abort\n"
                "                         at the next op boundary\n"
                "                         (returns cancel_signaled),\n"
                "                         and the artefact lands in\n"
                "                         OUTPUTS with the cancel\n"
                "                         reason in errors.log.\n"
                "  DELETE /jobs           prune stale-running entries\n"
                "                         (DONE/.plan with no OUTPUTS).\n"
                "  DELETE /jobs/all       wipe everything. NOT a force\n"
                "                         cancel.\n"
                "  GET    /outputs/<digest>.txt  sentinel manifest.\n"
                "  GET    /outputs/<digest>.tar  full artefact tar.\n"
                "  DELETE /outputs/<digest>      cleanup once the\n"
                "                                agent has fetched\n"
                "                                the artefact:\n"
                "                                removes the .txt\n"
                "                                and .tar AND the\n"
